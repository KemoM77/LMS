    ...
    open && (
      <div
        className={classNames({
          ...
          'sticky top-16 z-10 max-[1100px]:fixed max-[1100px]:z-20  max-[600px]:top-14': true, // positioning
          'min-h-[calc(100vh_-_64px)] max-[600px]:min-h-[calc(100vh_-_54px)]': true,
      ...
      >
        <nav className="sticky top-0">
          ...
          <ul className="flex flex-col gap-2 py-2">
            {navItems.map((item, index) => {
              return (
               ( (currentUser?.isLibrarian === item.forLabrarian || item.forLabrarian === item.forUsers )  && <Link key={index} href={item.href}>
                  <li
                    className={classNames({
        ...
        <div className="border-t border-t-indigo-800 p-4 ">
          {!loading ? (
            ...
            rounded-full duration-300 hover:scale-125"
              />
              <div className="flex flex-row items-center justify-between gap-12 min-[800px]:gap-20">
                ...
                <LogoutIcon tabIndex={0} onClick={handleSignout} className="cursor-pointer opacity-100 duration-300 " />
              </div>
            </div>
          ) : (
